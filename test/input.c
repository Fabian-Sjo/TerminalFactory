...
int main()
{
	...
	*/
	...
	{
		int nrOfEvents;
		GetNumberOfConsoleInputEvents(hin, &nrOfEvents);
		
		if (nrOfEvents == 0)
		continue;
		//printf("nrOfEvents %d\n", nrOfEvents);
		// if(!(GetNumberOfConsoleInputEvents(hin, &Events) && Events))
		//	continue;
		drawpixel(buttonX, buttonY, 1);
		...
		case MOUSE_EVENT: // mouse input
			...
			printf("Hello world at %d x %d ", InputRecord.Event.MouseEvent.dwMousePosition.X, InputRecord.Event.MouseEvent.dwMousePosition.Y);

			if (InputRecord.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED && 0)
			{
